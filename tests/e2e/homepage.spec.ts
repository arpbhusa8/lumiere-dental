...
test.describe("Homepage", () => {
  test("renders hero, treatments and CTA", async ({ page }) => {
    await page.goto("/");
    await expect(page).toHaveTitle(/Om Sai Dental Implant Center/);
    await expect(page.locator("h1").first()).toBeVisible();
    await expect(page.getByRole("link", { name: /book a consultation/i }).first()).toBeVisible();
    await expect(page.getByText(/Credentials & affiliations/i)).toBeVisible();
    await expect(page.getByRole("heading", { name: /Considered care, made/i })).toBeVisible();
  });
